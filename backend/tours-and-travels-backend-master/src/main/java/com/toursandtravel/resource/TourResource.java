...
import com.toursandtravel.service.ActivityService;
import com.toursandtravel.service.LocationService;
import com.toursandtravel.service.LodgingService;
import com.toursandtravel.service.MealService;
import com.toursandtravel.service.StorageService;
...
//		if (request == null || request.getTourId() == 0) {
		if (request == null || request.getTourId() == null || request.getTourId().isEmpty()) {
			response.setResponseMessage("missing request body");
			response.setSuccess(false);
	...
	public ResponseEntity<ActivityOrMealResponseDto> addTourMeal(AddTourActivityOrMeal request) {
		...
		if (request == null || request.getTourId() == null) {
			response.setResponseMessage("missing request body");
			response.setSuccess(false);
...
//		}
		if (request.getFromLocationId() != null
				&& !request.getFromLocationId().trim().isEmpty()
				&& (dbTour.getFromLocation() == null
				|| !request.getFromLocationId().equals(dbTour.getFromLocation().getId()))) {
			dbTour.setFromLocation(this.locationService.getById(request.getFromLocationId()));
		}

		if (request.getToLocationId() != null
				&& !request.getToLocationId().trim().isEmpty()
				&& (dbTour.getToLocation() == null
				|| !request.getToLocationId().equals(dbTour.getToLocation().getId()))) {
			dbTour.setToLocation(this.locationService.getById(request.getToLocationId()));
		}

		if (request.getTransportId() != null
				&& !request.getTransportId().trim().isEmpty()
				&& (dbTour.getTransport() == null
				|| !request.getTransportId().equals(dbTour.getTransport().getId()))) {
			dbTour.setTransport(this.transportService.getById(request.getTransportId()));
		}

		if (request.getLodgingId() != null
				&& !request.getLodgingId().trim().isEmpty()
				&& (dbTour.getLodging() == null
				|| !request.getLodgingId().equals(dbTour.getLodging().getId()))) {
			dbTour.setLodging(this.lodgingService.getById(request.getLodgingId()));
		}
	...
	public ResponseEntity<CommonApiResponse> deleteTour(String tourId) {

		LOG.info("request received for deleting the  tour");

		CommonApiResponse response = new CommonApiResponse();

		if (tourId == null || tourId == null) {
	...
	public ResponseEntity<TourResponseDto> fetchActiveToursByLocations(String fromLocationId, String toLocationId) {
	...
	public ResponseEntity<TourResponseDto> getAllToursByTourGuide(String tourGuideId) {
	...
	public ResponseEntity<TourResponseDto> fetchTourByTourId(String tourId) {
