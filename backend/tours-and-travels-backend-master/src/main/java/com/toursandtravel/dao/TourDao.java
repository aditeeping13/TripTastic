...
public interface TourDao extends MongoRepository<Tour, String> {
	...
	List<Tour> findByStatusAndFromLocationAndToLocationAndStartDateGreaterThan(
			String status, Location fromLocation, Location toLocation, String startDate);

	List<Tour> findByStatusAndNameContainingIgnoreCaseAndStartDateGreaterThan(
			String status, String name, String startDate);
